...

{
  ...
  
  {
    ...
    for (config_index in 1:nrow(all_config))
    {
      ...
      for (r in round)
      {
        print(paste0('Case =',set,' algorithm=',algoname,' score=',scorename,' test=',testname,' alpha=',alphavalue,' Penalty=',Penalty,' Structure=',structureprior,' round=',r))
        
        if (resamsple)   train_number = sample_size*(total_fold-1)/total_fold
        if (!resamsple)   train_number = n*(total_fold-1)/total_fold
        
        set.seed(r)
        #dataset_truedag$`Negative attitude` <- NULL
        ...
        for(i in names(dataset)){
          ...
          dataset[[i]] <- factor(dataset[[i]], ordered=TRUE)      
        }
        feather <- sapply(dataset[,sapply(dataset, is.factor)], nlevels)
        
        
        for (feather_index in colnames(dataset))
        {
          if (feather[feather_index] ==1)
          {
            dataset <- dataset[,!names(dataset) %in% feather_index]
            dataset_test <- dataset_test[,!names(dataset_test) %in% feather_index]
            
          }
        }
        
        dataset_test <- dataset_truedag[!rownames(dataset) %in%  rownames(dataset_truedag), ]
        
        if (resamsple)    test_number = sample_size/total_fold
        if (!resamsple)   test_number = n/total_fold
        
        
        dataset_test <- dataset_test[sample(nrow(dataset_test),test_number, replace= TRUE), ]
        
        for(i in names(dataset_test)){
          dataset_test[[i]] <- droplevels(dataset_test[[i]])
          dataset_test[[i]] <- factor(dataset_test[[i]], ordered=TRUE)      
        }
        # 
        feather <- sapply(dataset_test[,sapply(dataset_test, is.factor)], nlevels)
        for (feather_index in colnames(dataset_test))
        {
          if (feather[feather_index] ==1)
          {
            dataset_test <- dataset_test[,!names(dataset_test) %in% feather_index]
      ...
      avg_perf<- rbind(avg_perf,avg_perf_temp)
    ...
    ##################### select algorithms with dynamic score AIC/MIC ##########################
    {
      ...
      if (tuning_score =='BDeu') tuning_score_col = 24-17
      if (tuning_score =='BIC') tuning_score_col = 25-17
      if (tuning_score =='AIC') tuning_score_col = 27-17
      if (tuning_score =='LL') tuning_score_col = 26-17
      if (tuning_score =='MI') tuning_score_col = 31-17
      if (tuning_score =='MI-sh') tuning_score_col = 32-17
      if (tuning_score =='avg_MI') tuning_score_col = 29-17
      if (tuning_score =='avg_MI-sh') tuning_score_col = 30-17
      ...
      for (config_index3 in 1:nrow(all_config))
      {
        ...
        if (all_config[config_index3,1] == 'mmhc')
        {
          ...
          write.csv(optimal_config, paste0('output/',set,'/bestconfig_SOS_v4_realworld_',tuning_score,"_",all_config[config_index3,1],'_',set,'.csv'), row.names=F)
    ...
    write.csv(avg_perf, paste0("output/",set,"/performance_SOS_dyn_v4_realworld_",length(round),"_",set,".csv"), row.names=F)
    ...
    write.csv(log2.txt, paste0("output/",set,"/runtime_SOS_dyn_v4_realworld_(",tuning_score,")_",set,".csv"), row.names=F)

...
##################### select algorithms with dynamic score AIC/MIC ##########################
{
  ...
  if (tuning_score =='BDeus') tuning_score_col = 32+3
  if (tuning_score =='avg_MI') tuning_score_col = 29+3
  if (tuning_score =='avg_MI-sh') tuning_score_col = 30+3
  ...
  for (config_index3 in 1:nrow(all_config))
  {
    ...
    if (all_config[config_index3,1] == 'mmhc')
    {
      ...
      optimal_config = sample(1:range_compare,1)
      #F1
  ...
  write.csv(avg_perf_combine,paste0("output/performance_SOS_dyn_random_notreverse_avg_10.csv"), row.names=F)
}

...
# SOS_dynamic v4 clean up all comments modified PC to performance_bic <- score(sample_dag_from_cpdag,dataset,type = "custom",fun=ebic,args=list(gamma_penalty=0))
...
for (tuning_score in tuning_score_list)
{
  ...
  { 
    ...
    for (n in samplesize_list)
    {
        ...
        for (r in round)
        {
          ...
          if (algoname =='FGS_tetrad')
          {
            ...
            
            {
              ...
              performance_bdes <- score(sample_dag_from_cpdag, dataset_test, type = "bde", iss= 1)
              
              performance <- c(performance_bde,performance_bic,performance_ll,performance_aic,free_param, performance_avg_mi,performance_bdes,performance_mi,performance_mi_sh)
            ...
            colnames(value) <- c("Avg_BDeu","Avg_EBIC(norm)","Avg_ll","Avg_aic","avg_free_param","Avg_mi","Avg_BDeu(standard)","total_mi","total_mi_sh")
          ...
          if (algoname =='hc')
          {
            ...
            
            {
              ...
              performance_bdes <- score(hc_runner, dataset_test, type = "bde", iss= 1)
          ...
          if (algoname =='mcmc')
          {
            ...
            
            {
              ...
              performance_bdes <- score(learnt_dag, dataset_test, type = "bde", iss= 1)
          ...
          if (algoname =='mmhc')
          {
            ...
            
            {
              ...
              performance_bdes <- score(mmhc_runner, dataset_test, type = "bde", iss= 1)
              performance <- c(performance_bde,performance_bic,performance_ll,performance_aic,free_param, performance_avg_mi,performance_bdes,performance_mi,performance_mi_sh)
      ...
      ##################### select algorithms with dynamic score AIC/MIC ##########################
      {
        ...
        for (config_index3 in 1:nrow(all_config))
        {
          ...
          if (all_config[config_index3,1] == 'mmhc')
          {
            ...
            write.csv(optimal_config, paste0('output/',set,'/bestconfig_SOS_v4.1_',tuning_score,"_",all_config[config_index3,1],'_',set,'_',n,'.csv'), row.names=F)
      ...
      write.csv(avg_perf, paste0("output/",set,"/performance_SOS_dyn_v4.1_test_(",tuning_score,")_avg_",length(round),"_",set,"_",n,".csv"), row.names=F)
      ...
      write.csv(log2.txt, paste0("output/",set,"/runtime_SOS_dyn_v4.1_(",tuning_score,")_",set,"_",n,".csv"), row.names=F)

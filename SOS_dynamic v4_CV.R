...
# SOS_dynamic v4_CV clean up all comments
...
latent <- c("")
#set_list = c('ASIA','ALARM','SPORTS','PROPERTY')
set_list = c('ASIA')
...
for (tuning_score in tuning_score_list)
{
 ...
 # all_config <- rbind(hc_config,mcmc_config,pcstable_config,mmhc_config)
  all_config <- rbind(hc_config)
  ...
  { 
    ...
    for (n in samplesize_list)
    {
      ...
      set.seed(n)
      dataset_truedag <- dataset_truedag[sample(nrow(dataset_truedag),), ]
      
      folds <- rep_len(1:total_fold, nrow(dataset_truedag))
      ...
      for (config_index in 1:nrow(all_config))
      {
        ...
        for (r in round)
        {
          ...
          fold <- which(folds == r)
          dataset <- dataset_truedag[-fold,]
          dataset_test <- dataset_truedag[fold,]
          
          
          V = ncol(dataset_truedag)
          ...
          {
            tic("strcuture runtime FGS_tetrad")
            if ( scorename=='bic') 
            {
            ...
            if ( scorename=='bdeu')  
            {
              tetradrunner <- tetradrunner(algoId = 'fges',
                                           ...
                                           dataType = 'discrete',
                                           scoreId =  'bdeu',
                                           samplePrior = structureprior,
                                           verbose = FALSE)
            }
            toc(log = TRUE, quiet = TRUE)
            list_G[[r]]<-tetradrunner
          ...
          if (algoname =='hc')
          {
            ...
            tic("strcuture runtime hc")
            
            if (scorename=='bdeu')  hc_runner <- hc(dataset,score = 'bde',iss= structureprior)
            if (scorename=='bic')  hc_runner <- hc(dataset,score = 'bic',k=Penalty)
            # if (scorename=='bic')  hc_runner <- hc(dataset,type = "custom",fun=ebic,args=list(gamma_penalty=Penalty))
            list_G[[r]]<-hc_runner
          ...
          if (algoname =='pc')
          {
            ...
            tic("strcuture runtime pc")
            pc_runner <- pc.stable(dataset,  test = testname, alpha = alphavalue, undirected = FALSE)
          ...
          if (algoname =='mcmc')
          {
            ...
            tic("strcuture runtime mcmc")
            myScore<-scoreparameters("bdecat",dataset,bdecatpar = list(chi = 0.05, edgepf = structureprior))
            MCMCchains<-list()
            set.seed(1)
          ...
          if (algoname =='mmhc')
          {
            ...
            tic("strcuture runtime mmhc")
            if (scorename=='bdeu')mmhc_runner <- mmhc(dataset,  restrict.args=list(test = testname, alpha = alphavalue),maximize.args = list(score = 'bde',iss= structureprior))
            if (scorename=='bic') mmhc_runner <- mmhc(dataset,  restrict.args=list(test = testname, alpha = alphavalue),maximize.args = list(score = 'bic',k=Penalty))
            
            
            #if (scorename=='bic') mmhc_runner <- mmhc(dataset,  restrict.args=list(test = testname, alpha = alphavalue),maximize.args = list(score = 'custom',fun=ebic,args=list(gamma_penalty=Penalty)))
            
            
            
            toc(log = TRUE, quiet = TRUE)
            list_G[[r]]<-mmhc_runner
        ...
        list_config_G[[config_index]] <-list_G
        saveRDS(list_config_G, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_avg_",length(round),"_",set,"_",n,".rds"))
        ...
        
        {
          ...
          if (algoname =='FGS_tetrad')
          {
            ...
            saveRDS(tetradrunner, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_dataset_truedag_(",algoname,")_avg_",config_index,"_",set,"_",n,".rds"))
          ...
          if (algoname =='hc')
          {
            ...
            #if (scorename=='bic') hc_runner <- hc(dataset_truedag,score = 'custom',fun=ebic,args=list(gamma_penalty=Penalty))
            saveRDS(hc_runner, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_dataset_truedag_(",algoname,")_avg_",config_index,"_",set,"_",n,".rds"))
          ...
          if (algoname =='pc')
          {
            ...
            saveRDS(pc_runner, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_dataset_truedag_(",algoname,")_avg_",config_index,"_",set,"_",n,".rds"))
          ...
          if (algoname =='mcmc')
          {
            ...
            toc(log = TRUE, quiet = TRUE)
            edge_amat<-as.matrix(getDAG(MCMCchains[[1]],amat = TRUE, cp = FALSE))
            saveRDS(edge_amat, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_dataset_truedag_(",algoname,")_avg_",config_index,"_",set,"_",n,".rds"))
          ...
          if (algoname =='mmhc')
          {
            ...
            saveRDS(mmhc_runner, file = paste0("outputG/",set,"/performance_SOS_dyn_v4_CV_dataset_truedag_(",algoname,")_avg_",config_index,"_",set,"_",n,".rds"))
      ...
      ##################### select algorithms with dynamic score AIC/MIC ##########################
      {
        ...
        for (config_index3 in 1:nrow(all_config))
        {
          ...
          if (all_config[config_index3,1] == 'mmhc')
          {
            ...
            write.csv(optimal_config, paste0('output/',set,'/bestconfig_SOS_v4_CV_',tuning_score,"_",all_config[config_index3,1],'_',set,'_',n,'.csv'), row.names=F)
      ...
      write.csv(avg_perf, paste0("output/",set,"/performance_SOS_dyn_v4_CV_test_(",tuning_score,")_avg_",length(round),"_",set,"_",n,".csv"), row.names=F)
      ...
      write.csv(log2.txt, paste0("output/",set,"/runtime_SOS_dyn_v4_CV_(",tuning_score,")_",set,"_",n,".csv"), row.names=F)
